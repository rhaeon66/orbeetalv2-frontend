...
export default function ClientList() {
  const { data: clients = [], error, isLoading, refetch } = useGetAdminClientsQuery();
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-500">
          Homepage client logos. Inactive clients stay hidden from visitors.
        </p>
        <Link href="/admin/clients/new" className="btn btn-teal btn-sm rounded-lg">
      ...
      {!isLoading && !error && clients.length === 0 && (
        <div className="card p-8 text-center">
          <p className="font-semibold text-ink-900">No clients yet</p>
          <p className="mt-1 text-sm text-ink-500">Add the first client logo to get started.</p>
        </div>
      )}
