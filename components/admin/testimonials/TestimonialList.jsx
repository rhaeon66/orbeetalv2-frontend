...
export default function TestimonialList() {
  const { data: testimonials = [], error, isLoading, refetch } =
    useGetAdminTestimonialsQuery();
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-500">
          Homepage testimonials. Inactive quotes stay hidden from visitors.
        </p>
        <Link href="/admin/testimonials/new" className="btn btn-teal btn-sm rounded-lg">
      ...
      {!isLoading && !error && testimonials.length === 0 && (
        <div className="card p-8 text-center">
          <p className="font-semibold text-ink-900">No testimonials yet</p>
          <p className="mt-1 text-sm text-ink-500">Add the first client quote to get started.</p>
        </div>
      )}
